...
execute if entity @e[tag=dummy,scores={warpedCooking=60}] run data merge entity @e[tag=hyphae,limit=1] {ArmorItems:[{},{},{},{id:"minecraft:warped_hyphae",count:1,components:{"minecraft:equippable":{slot:"head",equip_sound:"block.chorus_flower.death",allowed_entities:"armor_stand",swappable:false},"minecraft:enchantment_glint_override":true,"minecraft:custom_name":'{"bold":true,"color":"dark_purple","text":"Active Warped Core"}'}}]}
...
execute if entity @e[tag=dummy,scores={warpedCooking=81}] run schedule function e1:summon_well_keeper 3s
